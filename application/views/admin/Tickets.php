...
<div style="text-align: right;">
	<a href="<?php echo base_url();?>admin/Tickets/Addtickets" class="btn btn-success btn-icon">
		Add Tickets
	...
	<tbody>
		...
		?>
			...
			<tr class="odd gradeX">
				...
				<td><?php echo $value['TicketA']; ?></td>
				<td class="center">
				    <?php if($is_purchased->num_rows() > 0){
				    ?>
				    <a href="javascript:void(0)" data-id1="<?php echo $value['id'];?>" class="btn btn-orange">
        				Cant be Editable
        			</a>
				    <?php
				    }else{?>
        			<a href="<?php echo base_url()?>admin/Tickets/editticket/<?php echo $value['id']; ?>" class="btn btn-default btn-sm btn-icon icon-left">
        				<i class="entypo-pencil"></i>
        				Edit
        			</a>
...
<script type="text/javascript">
	$(document).on('click','.dlt',function(){
		...
		if(response == true){
			window.location.href = "<?php echo SURL1;?>Tickets/delete/"+id;
		}
	});
</script>
