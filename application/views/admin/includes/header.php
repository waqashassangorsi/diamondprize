...
<div class="row">
	...
	<div class="col-md-6 col-sm-8 clearfix">
		...
		<ul class="user-info pull-left pull-none-xsm" style="margin-top:3px">
		
						<!-- Profile Info -->
			<li class="profile-info dropdown"><!-- add class "pull-right" if you want to place this from right -->
				<a href="#" class="dropdown-toggle" data-toggle="dropdown">
					<img src="<?php echo $user->dp ?>" alt="" class="img-circle" width="44" height="44" />
					<?php echo ucwords($user->fname); ?>
				</a>
...
<?php 
$this->db->query("DELETE t1 FROM carts t1 INNER JOIN carts t2 WHERE t1.id < t2.id AND t1.tickets = t2.tickets");

$allcarts = $this->db->query("select * from carts")->result_array();
foreach($allcarts as $key=>$value){
    $tickets_explode = explode(",",$value['tickets']);
    foreach($tickets_explode as $key=>$tickenoo){
       
        $query = $this->db->query("select * from ticket_purchased where lottery_id='".$value['lottery_id']."' and ticket_no='$tickenoo'");
        if($query->num_rows() > 0){
            $this->db->query("delete from carts where id='".$value['id']."'");
        }
    }
